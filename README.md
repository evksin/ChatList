...
3. **Если у вас проблемы с SSL**, временно отключите проверку SSL (только для диагностики):
...
4. **Если вы работаете через VPN**, добавьте в `.env`:
   ```
   USE_VPN=true
   ```
   Это увеличит таймауты для более стабильной работы через VPN.
...
**Важно:** Если у вас установлено несколько версий Python, рекомендуется использовать Python 3.12 или новее для лучшей поддержки SSL. Проверьте версию:

```powershell
python --version
```

Если используется Python 3.9 или старше, запустите с Python 3.12:
...
- Убедитесь, что API-ключ правильно указан в файле `.env`
- Ключ должен начинаться с `sk-or-v1-...`
- Если получаете ошибку 403, проверьте:
  1. Правильность API-ключа в `.env`
  2. Что файл `.env` находится в корне проекта
  3. Что ключ не содержит лишних пробелов или кавычек
  4. Перезапустите приложение после изменения `.env`

**Ошибка "unsupported_country_region_territory":**

Если вы получаете ошибку о неподдерживаемом регионе, это означает, что модель OpenAI недоступна в вашей стране. В этом случае:

- Используйте другие модели через OpenRouter, которые доступны в вашем регионе
- Популярные альтернативы: `meta-llama/Llama-2-70b-chat-hf`, `anthropic/claude-3-haiku`, `google/gemini-pro` и др.
